...
function DrinkInfo() {
  ...
  useEffect(() => {
    const fetchDrink = async () => {
      ...
      try {
        ...
        setError("Failed to load drink info");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url(${drink?.image && drink?.image.url ? (drink.image.url.startsWith('http') ? drink.image.url : `http://cinemate-001-site1.jtempurl.com/${drink.image.url}`) : '/images/logo1.png'})`,
                ...
                <button
                  ...
                  className={`px-14 py-2 rounded-full text-sm font-semibold border transition-all duration-300 ${
                    size === sizeOption
                      ...
                      : 'bg-transparent text-gray-300 border-gray-500 border-[1px] border-solid hover:bg-beige3 hover:text-gray-900 hover:shadow-lg'
                  }`}
                >
                  {sizeOption}
                </button>
