...
function Drinks() {
  ...
  const paginatedDrinks = drinks.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
        ...
        <div className="min-height-screen containermx-auto px-12 my-6">
          <div className="grid xl:grid-cols-4 2xl:grid-cols-5 lg:grid-cols-3 sm:grid-cols-2 gap-6 min-height-screen containermx-auto px-20 my-6">
            {paginatedDrinks.map((drink) => {
                // معالجة رابط الصورة بنفس منطق DrinkSlice
                let imgSrc = drink?.image?.url || '';
                if (!imgSrc || imgSrc === 'default-movie.jpg') {
                  ...
                  imgSrc = `/images/drinks/${imgSrc}`;
                }
                return (
                  <div key={drink.id} className="border border-border p-1 hover:scale-95 transition relative rounded-2xl overflow-hidden">
                    ...
                    <div className="absolute flex-btn gap-2 bottom-0 right-0 left-0 bg-main bg-opacity-60 text-white px-4 py-3">
                      <div>
                        <h3 className="font-semibold truncate">{drink.englishName}</h3>
                        <div className="flex flex-wrap gap-2 mt-1">
                          {drink.variants && drink.variants.length > 0 && drink.variants.map(variant => (
